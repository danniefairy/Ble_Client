...
public class Peripheral extends Activity implements ServiceFragmentDelegate {
  ...
  private final BluetoothGattServerCallback mGattServerCallback = new BluetoothGattServerCallback() {
    ...
    public void onCharacteristicWriteRequest(BluetoothDevice device, int requestId,
      ...
      //用runOnUiThread解決非main thread的 view問題

      //輸出接收資料
      try {
        final String doc = new String(value, "UTF-8");
        Log.v(TAG, "Characteristic Write request: " + Arrays.toString(value) + doc + value.length);
        runOnUiThread(new Runnable() {
          @Override
          public void run() {
            //輸入
            //TextView tv = (TextView) findViewById(R.id.tv);
            //tv.setText("Information:" + doc);

          }
        });
        //----activity to fragment
        HeartRateServiceFragment fragment01 = new HeartRateServiceFragment();
        Bundle bundle = new Bundle();
        bundle.putString("str",doc);
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    //start
    TextView tv = (TextView) findViewById(R.id.tv);
    tv.setText("Waiting for connection...");
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    if (item.getItemId() == R.id.action_disconnect_devices) {
      ...
      Toast.makeText(this, "Disconnect from device!",
  ...
  private void updateConnectedDevicesStatus() {
    ...
    //--
    if(String.valueOf(mBluetoothManager.getConnectedDevices(BluetoothGattServer.GATT).size())!="0")
    {
      TextView tv = (TextView) findViewById(R.id.tv);
      tv.setText("");
    }
